...
test.describe('Company Overview API Performance Tests - Using Enhanced Function', () => {
  ...
  test('should measure Company Overview API response time', { tag: ['@performance', '@fundamental'] }, async () => {
    const requestPayload = companyOverviewFactory();
    
    // const result = await getCompanyOverviewDataWithPerformance(requestPayload);
